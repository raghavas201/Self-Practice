// LEETCODE POTD 22-02-2025
// LEETCODE PROBLEM NUMBER 1028
...
// This C++ code reconstructs a binary tree from a given preorder
// traversal string where node values are separated by dashes indicating depth.
// The `recoverFromPreorder` function initializes the tree with the root node,
// while `buildTree` recursively attaches left and right children by parsing node values
// and their respective depths. `getVal` extracts numerical values, and `getDashLen`
// determines depth by counting dashes.
// The recursion ensures correct tree formation by tracking expected depth levels.